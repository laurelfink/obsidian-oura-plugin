...
export default class OuraApi {
    constructor(private oauth: OuraOAuth) {}
    ...
    public async getSleepData(theDate: string): Promise<OuraResponse | null> {
        ...
        const data = await this.oauth.request(`${OURA_API_URL}/daily_sleep?${params.toString()}`)
    ...
    public async getActivityData(theDate: string): Promise<OuraResponse | null> {
        ...
        const data = await this.oauth.request(`${OURA_API_URL}/daily_activity?${params.toString()}`)
    ...
    public async getReadinessData(theDate: string): Promise<OuraResponse | null> {
        ...
        params.set('start_date', start)
        params.set('end_date', theDate)
        const data = await this.oauth.request(`${OURA_API_URL}/daily_readiness?${params.toString()}`)
